...
<script type="text/javascript" src="<%=path%>/shopping-web/js/jquery-1.7.2.min.js"></script>
...
int id=user.getId();
  ...
  <div id="searchBox"> <!-- 搜索框 -->
	...
	<form name="findfom" class="formf" method="post" action="<%=path %>/FindGoodsAction"> 
    ...
    <hr>
    <% 
    String tname=(String)session.getAttribute("ThridClassify");//3类别名
   ArrayList<GoodsInfo> thridGoodsList = (ArrayList<GoodsInfo>)session.getAttribute("AllThridGoodsList");
   if(thridGoodsList==null||thridGoodsList.size()==0){ //必须判断，可能取空，报错
	   %>
	   <div align="center" style="height:300px">
	   <h2><%=tname %>还未上新，敬请期待......</h2></div>
	   <%
	}else{%>
	<div><!-- 美妆/个人护理等点击更多显示的大类别 -->
    <div class="goods1"><%=tname %></div>
    <ul class="goodsPic">
	
	<%
        for(int i=0; i<thridGoodsList.size() ;i++){
        	int gId=thridGoodsList.get(i).getGoods_id();
            String gName=thridGoodsList.get(i).getGoods_name();
        	double  gPrice=thridGoodsList.get(i).getGoods_price();
        	String gImgPath=thridGoodsList.get(i).getImg_path();//商品图片
        	%>
